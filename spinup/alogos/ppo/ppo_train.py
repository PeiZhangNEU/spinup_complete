...
import gym
import time
import spinup.alogos.ppo.core as core
from spinup.alogos.ppo.ppo import *
...
def train(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0,
        ...
        agent = ppo(env_fn=env_fn, actor_critic=actor_critic, ac_kwargs=ac_kwargs, steps_per_epoch=steps_per_epoch,
                ...
                lam=lam, target_kl=target_kl, logger_kwargs=logger_kwargs, device=device)
        env = env_fn()
        ...
        for epoch in range(epochs):
            ...
            agent.logger.log_tabular('DeltaLossPi', average_only=True)
            agent.logger.log_tabular('DeltaLossV', average_only=True)
            agent.logger.log_tabular('Entropy', average_only=True)
            agent.logger.log_tabular('KL', average_only=True)
            agent.logger.log_tabular('ClipFrac', average_only=True)
            agent.logger.log_tabular('StopIter', average_only=True)
