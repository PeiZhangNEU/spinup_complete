...
import gym
import time
import spinup.alogos.sac_discrete.core as core
from spinup.alogos.sac_discrete.sac_discrete import *
...
def train(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
         ...
         max_ep_len=200, logger_kwargs=dict(), save_freq=1, use_gpu=False):
        '''
        ...
        for t in range(total_steps): 
            ...
            if (t+1) % steps_per_epoch == 0:
                ...
                agent.logger.log_tabular('Q1Vals', with_min_and_max=True)
                agent.logger.log_tabular('Q2Vals', with_min_and_max=True)
                agent.logger.log_tabular('LossPi', average_only=True)
                agent.logger.log_tabular('LossQ', average_only=True)
                agent.logger.log_tabular('Alpha', average_only=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='sac_discrete')
