...
class trpo:
    ...
    def __init__(self, env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), 
                ...
                logger_kwargs=dict(),  device=None,
                mode=None
        ):
        self.device = device
        ...
        # 初始化logger，包括设置要保存谁的模型！保存的是一个类，同时保存了模型的方法
        self.logger = EpochLogger(**logger_kwargs)
        self.logger.save_config(locals())
        self.logger.setup_pytorch_saver(self.ac)  # 要保存什么的模型, 保存的是actorCritic这个整体。
...
if __name__ == "__main__":
    ...
    logger_kwargs = setup_logger_kwargs('trpo', 0)
    # env = gym.make('CartPole-v0')
    env = gym.make('Hopper-v2')

    agent = trpo(lambda : env, 
    ...
    # 产生两个obs数据
    obs = env.reset()
    obs1, r, d, _ = env.step(env.action_space.sample())
    obs_batch = np.zeros([2, env.observation_space.shape[0]], dtype=np.float32)
    obs_batch[0] = obs
    obs_batch[1] = obs1
    obs_data = torch.as_tensor(obs_batch).to(device)

    kl = agent.compute_kl(agent.old_pi, agent.ac.pi, obs_data)
    print(kl)
